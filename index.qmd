...
ggplot(dat_fc, aes(time_4_vs_0, time_8_vs_0)) +
      geom_point(alpha = .5) + 
      geom_smooth(method = 'lm', color = 'orange', se = FALSE, linewidth = .6)
...
mean_exp_by_time <- dat |>
 ...
 ggplot(mean_exp_by_time, aes(time, mean_exp, group = gene, color = gene)) + 
       geom_line() + 
       labs(y = 'mean y2 expression')

ggplot(mean_exp_by_time, aes(time, mean_exp, group = gene)) + 
       geom_line() +
       facet_wrap(~ gene) +
       labs(y = 'mean y2 expression')
```
...
ggplot(mean_exp_by_time_sex, aes(time, mean_exp, group = sex, color = sex)) + 
       geom_line() +
       facet_wrap(~ gene) +
       labs(y = 'mean y2 expression') + 
...
ggplot(mean_exp_by_time_sex, 
       aes(time, mean_exp, group = gene, color = gene)) +
       geom_line() + 
       facet_grid(sex ~ .) + 
       labs( y = 'mean y2 expression')
```